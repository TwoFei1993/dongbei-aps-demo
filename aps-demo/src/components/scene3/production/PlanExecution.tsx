...
export function PlanExecution() {
  ...
  const all = useMemo(() => loadKpiProduction(), [])
  const raw = useMemo(() => {
    if (productLine === '全部') return all.slice(0, 8)
    const found = all.filter((p) => p.productLine === productLine)
    return found.length ? found : all.slice(0, 1)
  }, [all, productLine])
